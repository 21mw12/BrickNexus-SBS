...
import { editRole, fetchRoleDetail, fetchPageTree, type PageTreeNode } from '../../../api/user'
import { fetchAssetTree, type AssetTreeNode } from '../../../api/asset.ts'
import type { AssetTypePermission, AssetIdPermissionNode } from '../../../api/user'

const props = defineProps<{ visible: boolean; roleId: string }>()
...
function ensureAssetPerm(assetId: string) {
  if (!assetIdPerms[assetId]) assetIdPerms[assetId] = { U: false, R: false, D: false, O: false }
...
// 从 AssetIdPermissionNode 树递归回填权限（查询响应为树形，需递归处理 sub_assets）
function fillPermsFromTree(nodes: AssetIdPermissionNode[]) {
  for (const node of nodes) {
    const perm = ensureAssetPerm(node.asset_id)
    const p = node.permission
    if (p.includes('R')) perm.R = true
    if (p.includes('U')) perm.U = true
    if (p.includes('D')) perm.D = true
    if (p.includes('O')) perm.O = true
    if (node.sub_assets) fillPermsFromTree(node.sub_assets)
  }
}
...
watch(() => props.visible, async (v) => {
  if (v && props.roleId) {
    ...
    try {
      const [detail, pages, assets] = await Promise.all([
        fetchRoleDetail(props.roleId), fetchPageTree(), fetchAssetTree(),
      ])
      form.name = detail.name
      form.describe = detail.describe || ''
      selectedPageIds.value = new Set(detail.page_ids || [])
      ...
      if (detail.asset_permission) {
        // 资产类型权限：仅 C（创建）
        for (const p of detail.asset_permission.part_asset_type || []) {
          const t = assetTypePerms[p.type as AssetTypeKey]
          if (t) {
            if (p.permission.includes('C')) t.C = true
          }
        }
        // 资产实例权限：树形响应，递归填充
        if (detail.asset_permission.part_asset_id) {
          fillPermsFromTree(detail.asset_permission.part_asset_id)
        }
      }

      // 自动展开资产树：展开所有有权限的节点及其祖先
      const expandSet = new Set<string>()
      for (const [assetId, perm] of Object.entries(assetIdPerms)) {
        if (perm.R || perm.U || perm.D || perm.O) {
          expandSet.add(assetId)
          let parent = parentMap.value[assetId]
          while (parent) {
            expandSet.add(parent)
            parent = parentMap.value[parent]
          }
        }
      }
      assetTreeExpanded.value = expandSet
    } catch (e: any) {
      error.value = e?.message || '加载角色信息失败'
    } finally {
  ...
  try {
    ...
    await editRole(props.roleId, {
      ...
      asset_permission: (part_asset_type.length || part_asset_id.length)
        ? { part_asset_type: part_asset_type.length ? part_asset_type : undefined, part_asset_id: part_asset_id.length ? part_asset_id as any : undefined }
        : undefined,
    })
    emit('updated')
    emit('close')
...
<template>
  <div class="modal-overlay" v-if="visible" @click.self="emit('close')">
    <div class="modal-card form-modal">
      <div class="modal-header"><h3>编辑角色</h3></div>

      <div class="modal-body" v-if="fetchLoading"><p class="tree-hint">加载角色信息...</p></div>
      ...
      <div class="modal-footer" v-if="!fetchLoading">
        ...
        <button class="btn btn-primary" @click="confirm" :disabled="loading">{{ loading ? '保存中...' : '保存' }}</button>
      </div>
    </div>
  </div>
</template>
