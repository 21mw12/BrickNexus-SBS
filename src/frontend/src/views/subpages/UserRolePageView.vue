...
import { fetchRolePage, deleteRole, type RoleInfo } from '../../api/user.ts'
import CreateRoleModal from '../modals/role/CreateRoleModal.vue'
import EditRoleModal from '../modals/role/EditRoleModal.vue'
...
async function search(page = 1) {
  ...
  try {
    const result = await fetchRolePage(page, pageSize.value, {
      name: queryForm.name || undefined,
      describe: queryForm.describe || undefined,
    })
...
<template>
  <main class="page-content">
    <section class="workspace">
      ...
      <div class="page-body">
        <div class="content-card">
          ...
          <table class="data-table" v-if="roles.length > 0">
            <thead>
              <tr>
                <th>角色名</th>
                <th>描述</th>
            ...
            <tbody>
              <tr v-for="row in roles" :key="row.role_id">
                <td>{{ row.name }}</td>
                <td>{{ row.describe }}</td>
          ...
          <!-- 空状态 -->
          <div class="empty-state" v-else-if="roles.length === 0">
            <p>暂无数据</p>
          </div>
          </div>
    ...
    <ConfirmModal
      ...
      title="删除角色"
      :message="`确定要删除角色「${deleteTarget?.name ?? ''}」吗？此操作不可恢复。`"
      confirmText="删除"
      :danger="true"
      :error="deleteError"
